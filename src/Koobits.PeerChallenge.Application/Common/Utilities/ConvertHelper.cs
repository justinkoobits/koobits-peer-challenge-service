...
namespace Koobits.PeerChallenge.Application.Common.Utilities
{
    public sealed class ConvertHelper
    {
        ...
        public static PropertyHelper PropertyHelper { get; private set; }
        ...
        //Another method needs to be added before this method in case of differences in the selected timezone and the country from where the time was selected
        public static DateTime ChangeTimeZone(DateTime SourceDatetime, string TargetTimezone)
        {
            var utcTime = SourceDatetime.ToUniversalTime();
            var timeInfo = TimeZoneInfo.FindSystemTimeZoneById(TargetTimezone);
            var ConvertedDateTime = TimeZoneInfo.ConvertTimeFromUtc(utcTime, timeInfo);
        ...
        {
            var result = ToDate(value, format, formatProvider);
        ...
        {
            var result = ToDate(value, format);
